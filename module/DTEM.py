...
class DTEM(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DTEM, self).__init__()
        ...
        self.h_fusion = SKFF(out_channels, height=3, reduction=8)
    ...
    def forward(self, x):
        ...
        x_RC = self.h_fusion([x_HL, x_LH, x_HH])
        x_LH = self.cross_attention0(x_RC, x_LH)
        x_HL = self.cross_attention0(x_RC, x_HL)
        ...
        x_HH = self.conv_HH(torch.cat((x_HH_LH, x_HH_HL), dim=1))
